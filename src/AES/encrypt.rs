const NK: usize = 4; // Number of 32-bit words in the key (AES-128 has Nk=4)
const NR: usize = 10; // Number of rounds (AES-128 has Nr=10)
const SBOX: [u8; 256] = [
     ];

// Perform substitution on each byte
fn sub_bytes (state: &mut [u8; 16]) {
    for i in 0..16 {
        state[i] = SBOX[state[i] as usize];
    }
}

// Perform row shifts
fn shift_rows(state: &mut [u8; 16]) {
    // Row 1: No shift

    // Row 2: Shift left by 1
    let tmp = state[1];
    state[1] = state[5];
    state[5] = state[9];
    state[9] = state[13];
    state[13] = tmp;

    // Row 3: Shift left by 2
    let tmp1 = state[2];
    let tmp2 = state[6];
    state[2] = state[10];
    state[6] = state[14];
    state[10] = tmp1;
    state[14] = tmp2;

    // Row 4: Shift left by 3
    let tmp = state[3];
    state[3] = state[15];
    state[15] = state[11];
    state[11] = state[7];
    state[7] = tmp;
}

// Multiplication in GF(2^8)
fn gmul(a: u8, b: u8) -> u8 {
    let mut a = a;
    let mut b = b;
    let mut p = 0;
    for _ in 0..8 {
        if (b & 1) != 0 {
            p ^= a;
        }
        let high_bit_set = (a & 0x80) != 0;
        a <<= 1;
        if high_bit_set {
            a ^= 0x1b; // Polynomial for AES (x^8 + x^4 + x^3 + x + 1)
        }
        b >>= 1;
    }
    p
}

pub fn xtime(b: u8) -> u8 {
    (b << 1) ^ ((b >> 7) * 0x1B)
}

// Perform column mixing
pub fn mix_columns (s: &mut [u8; 16]) {
    for c in 0..4{
        let t = s[4*c] ^ s[4*c+1] ^ s[4*c+2] ^ s[4*c+3];
        let temp = s[4*c];
        s[4*c]   ^= xtime(s[4*c]   ^ s[4*c+1]) ^ t;
        s[4*c+1] ^= xtime(s[4*c+1] ^ s[4*c+2]) ^ t;
        s[4*c+2] ^= xtime(s[4*c+2] ^ s[4*c+3]) ^ t;
        s[4*c+3] ^= xtime(s[4*c+3] ^ temp)     ^ t;
    }
}


pub fn add_round_key (state: &mut [u8; 16], r_key: &[u8; 176], round: usize){
    for c in 0..16 {
        state[c] ^= r_key[16*round + c];
    }
}

pub fn key_expansion(key: &[u8]) -> [u8; 176] {
    // Predefined Round constants for AES key expansion
    let rcon: [u8; 10]=[0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36];
    let mut w: [u8; 176] = [0; 176];

    // Copy the initial key into the first Nk words of the expanded key
    for c in 0..16 {
        w[c] = key[c];
    }

    // Perform key expansion to generate the remaining words
    let mut i = NK;
    let mut temp: [u8; 4] = [0; 4];
    while i < 4 * NR + 4 {
        // Copy the previous word into temp
        temp[0] = w[4*(i-1)];
        temp[1] = w[4*(i-1) + 1];
        temp[2] = w[4*(i-1) + 2];
        temp[3] = w[4*(i-1) + 3];

        // Apply transformations if at the start of a round
        if i % NK == 0 {
            // Rotate the bytes in temp
            let t = temp[0];
            temp[0] = SBOX[temp[1] as usize] ^ rcon[(i/NK) - 1];
            temp[1] = SBOX[temp[2] as usize];
            temp[2] = SBOX[temp[3] as usize];
            temp[3] = SBOX[t as usize];
        }

        // XOR with the word NK positions earlier
        w[4*i]     = w[4*(i-NK)]     ^ temp[0];
        w[4*i + 1] = w[4*(i-NK) + 1] ^ temp[1];
        w[4*i + 2] = w[4*(i-NK) + 2] ^ temp[2];
        w[4*i + 3] = w[4*(i-NK) + 3] ^ temp[3];
        i += 1;
    }
    w
}

//AES encryption function
pub fn cipher (state: &mut [u8;16], exp_key: &[u8; 176]) {
    // Initial round key addition.
    add_round_key(state, exp_key, 0);

    // 9 main rounds.
    for round in 1..10 {
        sub_bytes(state);
        shift_rows(state);
        mix_columns(state);
        add_round_key(state, exp_key, round);
    }
    
    // Final round (no MixColumns).
    sub_bytes(state);
    shift_rows(state);
    add_round_key(state, exp_key, 10);
}